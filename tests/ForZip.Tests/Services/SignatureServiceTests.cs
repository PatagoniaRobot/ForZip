...
public class SignatureServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task Sign_WithoutTsa_ReportsNoTimestamp()
    {
        var manifestPath = await CreateManifestAsync("{ \"data\": 1 }");
        var pfxPath = CreateSelfSignedPfx("CN=Perito Test");

        await _service.SignAsync(manifestPath, pfxPath, "test", tsaUrl: null, CancellationToken.None);
        var info = _service.Verify(manifestPath);

        Assert.True(info.Valid);
        Assert.Null(info.TimestampUtc);
        Assert.Null(info.TimestampValid);
    }

    [Fact]
    public async Task Sign_TsaUnreachable_StillWritesValidSignature_AndThrows()
    {
        var manifestPath = await CreateManifestAsync("{ \"data\": 1 }");
        var pfxPath = CreateSelfSignedPfx("CN=Perito Test");

        // Puerto sin servicio: la TSA no responde, pero la firma debe quedar en disco
        await Assert.ThrowsAsync<TimestampUnavailableException>(() =>
            _service.SignAsync(manifestPath, pfxPath, "test",
                "http://127.0.0.1:9/tsa", CancellationToken.None));

        Assert.True(_service.IsSignaturePresent(manifestPath));

        var info = _service.Verify(manifestPath);
        Assert.True(info.Present);
        Assert.True(info.Valid);
        Assert.Null(info.TimestampUtc); // firmado, pero sin sello de tiempo
    }
