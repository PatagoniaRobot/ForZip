...
public class SignCommand
{
    ...
    public async Task<int> ExecuteAsync(CommandParser parser)
    {
        ...
        var timestampUrl = parser.GetOption("-t", "--timestamp-url");

        if (string.IsNullOrEmpty(manifestPath) || string.IsNullOrEmpty(certPath))
        {
            Console.WriteLine("Uso: forzip sign -m <manifiesto.json> -c <certificado.pfx> [-p <password>] [-t <url TSA RFC 3161>]");
            return 1;
        }
        ...
        try
        {
            await _signatureService.SignAsync(manifestPath, certPath, certPassword, timestampUrl, CancellationToken.None);
        }
        catch (Core.Services.TimestampUnavailableException ex)
        {
            WriteColored(ConsoleColor.Yellow, $"Aviso: la firma se generó SIN sello de tiempo. {ex.Message}");
        }
        catch (Exception ex)
        {
        ...
        if (info.Valid)
        {
            ...
            {
                Console.WriteLine($"  Fecha de firma declarada (UTC): {info.SignedAtUtc.Value:yyyy-MM-ddTHH:mm:ssZ}");
            }
            if (info.TimestampUtc.HasValue)
            {
                var validity = info.TimestampValid == true ? "válido" : "NO VÁLIDO";
                Console.WriteLine($"  Sello de tiempo RFC 3161 (UTC): {info.TimestampUtc.Value:yyyy-MM-ddTHH:mm:ssZ} ({validity})");
                if (!string.IsNullOrEmpty(info.TimestampAuthority))
                {
                    Console.WriteLine($"  Autoridad de sellado (TSA): {info.TimestampAuthority}");
                }
            }
        }
