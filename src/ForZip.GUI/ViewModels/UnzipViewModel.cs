...
public partial class UnzipViewModel : ViewModelBase
{
    ...
    private async Task DecompressAsync(CancellationToken ct)
    {
        ...
        _logService.Info(string.Format(_localization.Get("log_extract_start"), Path.GetFileName(ZipFilePath)));
        _logService.Info(string.Format(_localization.Get("log_extract_dest"), OutputDirectory));
        _logService.Info(string.Format(_localization.Get("log_encryption"),
            string.IsNullOrEmpty(Password) ? _localization.Get("log_no") : "AES-256"));
        ...
        try
        {
            ...
            _logService.Success(string.Format(_localization.Get("log_extract_done"), OutputDirectory));
        }
        catch (OperationCanceledException)
        {
            StatusMessage = _localization.Get("operation_cancelled");
            _logService.Warning(_localization.Get("log_extract_cancelled"));
        }
        catch (Exception ex)
        {
                ...
                ex.Message.Contains("contraseña", StringComparison.OrdinalIgnoreCase))
            {
                ...
                _logService.Error(_localization.Get("error_wrong_password"));
            }
            else
            {
                StatusMessage = string.Format(_localization.Get("error_io"), ex.Message);
                _logService.Error(string.Format(_localization.Get("log_extract_error"), ex.Message));
            }
        }
        finally
        {
